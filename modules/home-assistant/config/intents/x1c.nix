    ...
    X1C_Druckdauer = {
      ...
      script.speech.text = ''
        ...
        {% if st == 'running' or st == 'pause' %}
          ...
          {% endif %}
          {% if st == 'pause' %}
            Der Drucker ist pausiert, mit noch {{ t }} und Layer {{ lay }} von {{ tot }} übrig.
          {% else %}
            Der Drucker druckt noch {{ t }}, Layer {{ lay }} von {{ tot }}.
          {% endif %}
        {% elif st == 'finish' %}
          Der Druck ist fertig.
        {% elif st == 'failed' %}
          Der letzte Druck ist fehlgeschlagen.
        {% elif st == 'prepare' %}
          Der Drucker bereitet sich gerade vor.
        {% elif st == 'offline' or st == 'unknown' or st == 'unavailable' %}
          Der Drucker ist offline.
        {% else %}
          Der Drucker ist im Leerlauf.
        {% endif %}
      '';
    };
  };
