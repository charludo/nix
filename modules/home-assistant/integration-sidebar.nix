...
in
{
  options.hass.sidebar = lib.mkOption {
    default = [ ];
    description = "Sidebar item order and visibility, written to www/custom-sidebar-config.yaml";
  ...
  config = lib.mkIf (cfg.sidebar != [ ]) {
    systemd.tmpfiles.rules =
      let
        sidebarCfg = (pkgs.formats.yaml { }).generate "custom-sidebar-config.yaml" {
      ...
      in
      [
        "L+ ${config.services.home-assistant.configDir}/www/custom-sidebar-config.yaml - - - - ${sidebarCfg}"
      ];
  };
}
