{ pkgs, ... }:
let
  lgtv = pkgs.writers.writePython3Bin "lgtv" { libraries = [ pkgs.python312Packages.flask ]; doCheck = false; }/* python */ ''
...
in
{
  ...
  environment.systemPackages = [ pkgs.python312Packages.aiopylgtv lgtv ];
